...
def validate_oauth():
	...
	if authorization_header and authorization_header[0].lower() == "bearer":
		...
		parsed_url = urlparse(r.url)
		access_token = {"access_token": token}
		uri = parsed_url.scheme + "://" + parsed_url.netloc + \
			parsed_url.path + "?" + urlencode(access_token)
		http_method = r.method
		body = r.get_data()
		headers = r.headers
...
def validate_auth_via_api_keys():
	...
	"""
	try:
		authorization_header = frappe.get_request_header("Authorization", None).split(
			" ") if frappe.get_request_header("Authorization") else None
		if authorization_header and authorization_header[0] == 'Basic':
			token = frappe.safe_decode(base64.b64decode(
				authorization_header[1])).split(":")
