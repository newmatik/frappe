...
def create_letmeship_shipment(pickup_address, delivery_address, shipment_parcel, description_of_content,
	...
	try:
		...
		if 'shipmentId' in response_data:
			...
			awb_number = ''
			url = 'https://api.test.letmeship.com/v1/shipments/{id}'.format(id=response_data['shipmentId'])
...
def get_letmeship_label(shipment_id):
	...
	url = 'https://api.test.letmeship.com/v1/shipments/{id}/documents?types=LABEL'\
...
def get_letmeship_tracking_data(shipment_id):
	...
	try:
		url = 'https://api.test.letmeship.com/v1/tracking?shipmentid={id}'.format(id=shipment_id)
